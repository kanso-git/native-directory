...
/* eslint no-restricted-syntax:"no-error" */

import axios from 'axios';
import { BDL_SECURITY_TOKEN, BDL_SECURITY_TOKEN_VAL, API_BDL, LOCAL, BUILDING,BDL_LOADED, BDL_LOADING, BDL_ERROR } from 'react-native-dotenv';
    ...
    // search bilune building minimum query length is 3
      if (query.trim().length > 2) {
        ...
        for (let b of buildings) {
          try{
            const base64Image = await getBase64Image(`${API_BDL}/batiments/${b.id}/photo/mini`);
      ...
      if (data.state === BDL_LOADED) {
        ...
        for (let l of locals) {
          try{
            const base64Image = await getBase64Image(`${API_BDL}/locaux/${l.attributes.LOC_ID}/photo/mini`);
            if(base64Image && base64Image.length > 100 ){
